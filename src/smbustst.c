...
#include <sys/ioctl.h>	//+V1.2.6 complie warring fixed
    ...
    while ((opt = getopt(argc, argv, "+SIvrwh:M:d:c:o:a:")) != -1) {
        ...
        switch (opt) {
            case 'S':   //Scan NIC
                ...
                inhibit_error = 1;
                break;
            case 'M':   //ModelName  Bypass On Card
            ...
            case 'w': //write bypass
                ...
                scan_mode = 5;
                break;
            case 'd':  //destination  = r/w address
                sscanf(optarg, "%x", &slave_address);
                break;
            case 'c': //bypass cmd
                sscanf(optarg, "%x", &reg_no);
                break;
            case 'o': //Output = write data
...
#define MODEL_GET_THROUGH_NUM	13			//+v1.2.9 Add NCB-6210 support
model_method_t model_get_through_tbl[MODEL_GET_THROUGH_NUM+1]= {
	{"MB-887X"    ,"Mode1", 0x73, 0x00},
	{"MB-8877EXT" ,"Mode2", 0x73, 0x00},
	{"MB-8865EXT" ,"Mode3", 0x73, 0x00},
	{"MB-7582EXT" ,"Mode4", 0x73, 0x00},
	{"MB-7583"    ,"Mode5", 0x73, 0x00},
	{"MB-8771EXT" ,"Mode6", 0x73, 0x00},
	{"MB-8895"    ,"Mode7", 0x72, 0x00},
	{"MB-MFI20K"  ,"Mode8", 0x72, 0x00},  
	{"MB-9655"    ,"Mode9", 0x70, 0x00},
	{"NCB-5510EXT","Mode10",0x72, 0x00},
	{"NCB-5220"   ,"Mode11",0x73, 0x00},	//+v1.2.7 Add NCB-5220 support
	{"NCB-2510"   ,"Mode12",0x72, 0x00},	//+v1.2.7 Add NCB-2510 support
	{"NCB-6210"   ,"Mode13",0x73, 0x00},	//+v1.2.9 Add NCB-6210 support
	{0, 0, 0}
};
...
int get_through_i2c_bus( int fd ,int slave_addr, int v)
{
    ...
    while ( model_ptr->switch_addr != 0 ) {
		if(!strcmp(model_name, model_ptr->model)||
		   !strcmp(model_name, model_ptr->simple_model)) {
                                    
            if((strcmp(model_ptr->model,"MB-MFI20K")==0)||
               (strcmp(model_ptr->simple_model,"Mode8")==0)) //PCA9548
			{                                                        
  ...
  else if((slave_addr==0x33)||(slave_addr==0x34)||(slave_addr==0x35)||(slave_addr==0x36)||(slave_addr==0x37)||(slave_addr==0x38)||(slave_addr==0x39))
  {
					 model_ptr->switch_data=0x10;  //channel-03
  }
                           
				else
				{
				    if (v) {
						fprintf(stderr, "MB-MFI20K wrong Slave Address\n");
					}
					return -1;
				} 
            ...
            fprintf(stdout, "Get through first I2C Bus successful\n");
               ...
               second_layer_addr=0x74;
                             
                if(slave_addr==0x33)  //0x30
				{
					model_ptr->switch_data=0x01;  //channel01
				}
				else if(slave_addr==0x34)  //0x31
				{
					model_ptr->switch_data=0x02;  //channel02
				}
				else if(slave_addr==0x35)  //0x32
				{
					model_ptr->switch_data=0x04;  //channel03
				}
				else if(slave_addr==0x36)  //0x33
				{
					model_ptr->switch_data=0x08;  //channel04
				}
				else if(slave_addr==0x37)  //0x34
				{
					model_ptr->switch_data=0x10;  //channel05
				}
  		        else if(slave_addr==0x38)  //0x35
	             		{
					model_ptr->switch_data=0x20;  //channel06
				}
 			    else if(slave_addr==0x39)  //0x36
				{
					model_ptr->switch_data=0x00;  //channel07
				}
       ...
       else  if((strcmp(model_ptr->model,"MB-7583")==0)||
                (strcmp(model_ptr->simple_model,"Mode5")==0))
			{
				if((slave_addr==0x30))
				{                     
	...
	/*		    	if(i2c_smbus_write_byte(fd, second_layer_addr, model_ptr->switch_data, 0, NULL)) {
		  ...
		  else if((strcmp(model_ptr->model,"MB-7582EXT")==0)||
                  (strcmp(model_ptr->simple_model,"Mode4")==0))  //PCA9545
			{
				if((slave_addr==0x30)||(slave_addr==0x31)||(slave_addr==0x32))
				{
            ...
            else if((strcmp(model_ptr->model,"MB-8865EXT")==0)||
                    (strcmp(model_ptr->simple_model,"Mode3")==0))  //PCA9545
			{
				if((slave_addr==0x30)||(slave_addr==0x31)||(slave_addr==0x32))
				{
				    /* Slot 1~3 is located at bottom module, left to right */
...
//					second_layer_addr=0x73;  //PCA9545 address
			...
			else if((strcmp(model_ptr->model,"MB-887X")==0)||
               (strcmp(model_ptr->simple_model,"Mode1")==0)) //PCA9545
			{
			...
			else if((strcmp(model_ptr->model,"MB-9655")==0)||
                    (strcmp(model_ptr->simple_model,"Mode9")==0))  //PCA9548
			{
			...
			else if((strcmp(model_ptr->model,"MB-8877EXT")==0)||
                    (strcmp(model_ptr->simple_model,"Mode2")==0))  //PCA9548(111-0011)
			{
				if(slave_addr==48)  //0x30				
				{
                                 i2c_smbus_write_byte(fd, 0x77, 0x02, 0, NULL);
					model_ptr->switch_data=0x01;  //channel01
				}
				else if(slave_addr==49)  //0x31
				{
                                 i2c_smbus_write_byte(fd, 0x77, 0x04, 0, NULL);
					model_ptr->switch_data=0x01;  //channel02
				}
				else if(slave_addr==50)  //0x32
				{
                                 i2c_smbus_write_byte(fd, 0x77, 0x08, 0, NULL); 
					model_ptr->switch_data=0x01;  //channel03
				}
				else if(slave_addr==51)  //0x33
				{
                                 i2c_smbus_write_byte(fd, 0x77, 0x10, 0, NULL); 
					model_ptr->switch_data=0x01;  //channel04
				}
				else
				{
				    	if (v) {
						fprintf(stderr, "MB-8877EXT wrong Slave Address\n");
					}
					return -1;
				}
			}
	        	...
	        	else if((strcmp(model_ptr->model,"MB-8771EXT")==0)||
                        (strcmp(model_ptr->simple_model,"Mode6")==0)) //PCA9545
			{
				...
				{
				    if (v) {
						fprintf(stderr, "MB-8771EXT wrong Slave Address\n");
					}
					return -1;
				}
				if(i2c_smbus_write_byte(fd, model_ptr->switch_addr, model_ptr->switch_data, 0, NULL)) {
...
//---NCB-5510EXT-----------------------------------------------------------------------
	        	...
	        	else if((strcmp(model_ptr->model,"NCB-5510EXT")==0)||
               		    (strcmp(model_ptr->simple_model,"Mode10")==0))  //PCA9548
			{
				//TEST
				(void)i2c_smbus_write_byte(fd, 0x72, 0, 0, NULL);//MotherBoard
				(void)i2c_smbus_write_byte(fd, 0x73, 0, 0, NULL);//Expansion 
				//END

				if(slave_addr==0x30)
				{
					model_ptr->switch_addr=0x72;
					model_ptr->switch_data=0x01;  //channel-01
				}
				else if(slave_addr==0x31)
				{
				...
				{
				    if (v) {
						fprintf(stderr, "NCB-5510EXT wrong Slave Address\n");
					}
					return -1;
				}
			    	if(i2c_smbus_write_byte(fd, model_ptr->switch_addr, model_ptr->switch_data, 0, NULL)) {
...
//+v1.2.7 Add NCB-5220 support
//---------------------------------------------------------------------------      
	        	else if((strcmp(model_ptr->model,"NCB-5220")==0)||
                  (strcmp(model_ptr->simple_model,"Mode11")==0))  //PCA9545
			{
				if(slave_addr==0x30)
				{
				    /* Slot 1 is located at OnBoard module */
				    if (slave_addr == 0x30) {
				    	model_ptr->switch_data=0x08;  //channel-03 (SC3/SD3)
				    } 
				    if(i2c_smbus_write_byte(fd, model_ptr->switch_addr, model_ptr->switch_data, 0, NULL)) {
			            	if (v)
				        	fprintf(stderr, "Get through first I2C BUS FAILED\n");
				...
				else if((slave_addr==0x31)||(slave_addr==0x32))
				{
				    /* Slot 2-3 is located at module, left to right */
					model_ptr->switch_data=0x01;  //channel-00 (SC0/SD0)
				}
				else
				{
				    if (v) {
						fprintf(stderr, "NCB-5220 wrong Slave Address\n");
					}
					return -1;
				}
				if(i2c_smbus_write_byte(fd, model_ptr->switch_addr, model_ptr->switch_data, 0, NULL)) {
				...
				if (slave_addr==0x31)
				{
				    /* Slot 2 is located at top module, left */
					model_ptr->switch_data=0x02;  //channel-01
				}
				else if(slave_addr==0x32)
				{
...
//---------------------------------------------------------------------------                    
			else if((strcmp(model_ptr->model,"NCB-2510")==0)||
               	    (strcmp(model_ptr->simple_model,"Mode12")==0))  //PCA9548(1111-0100)
			{
				if(slave_addr==48)  //0x30
				{
					model_ptr->switch_data=0x01;  //channel0
				}
				else if(slave_addr==49)  //0x31
				{
					model_ptr->switch_data=0x20;  //channel04
				}
				else if(slave_addr==50)  //0x32
				{
					model_ptr->switch_data=0x40;  //channel05
				}
			else
			{
				if (v) {
						fprintf(stderr, "NCB-2510 wrong Slave Address\n");
				}
				return -1;
			}
...
//+V1.2.9 add new model for NCB-6210-------------------------------------------------------------
 			else if((strcmp(model_ptr->model,"NCB-6210")==0)||
               (strcmp(model_ptr->simple_model,"Mode13")==0)) //PCA9545
			{
				if(slave_addr==48)  //0x30
				{
					model_ptr->switch_data=0x01;  //channel-01
				...
				{
					model_ptr->switch_data=0x10;  //channel-05
				}
				else if(slave_addr==53)  //0x35
				{
					model_ptr->switch_data=0x20;  //channel-06
				}
				else if(slave_addr==54)  //0x36
				{
					model_ptr->switch_data=0x40;  //channel-07
				}
				else if(slave_addr==55)  //0x37
				{
					model_ptr->switch_data=0x80;  //channel-08
				}
				else
				{
				    	if (v) {
						fprintf(stderr, "NCB-6210 wrong Slave Address\n");
					}
					return -1;
				}
			}				 
 //---------------------------------------------------------------------------               
			else if((strcmp(model_ptr->model,"MB-8895")==0)||
               	    (strcmp(model_ptr->simple_model,"Mode7")==0))  //PCA9548(1111-0100)
			{
				if(slave_addr==48)  //0x30
				{
					model_ptr->switch_data=0x01;  //channel01
...
//      0x2F, //W83792D HW monitor
        ...
        0x38, //Lanner Bypass Module
        0x39, //Lanner Bypass Module
//	0x50, //EEPROM
//	0x51, //EEPROM
//	0x52, //EEPROM
//	0x53, //EEPROM
//	0x6A  //clock gen
	...
	for ( i = 0; i < sizeof(known_address); i++) {
		...
		if ( (scan_addr <= 0x39) && (scan_addr >=0x30)) {
			if (motherboard == 1) {
				if ( get_through_i2c_bus(fd, scan_addr, 0)) 
				scan_addr = 0x30;
			}
		}
            }
...
cmd_t info_tbl[]={
    ...
    {WDT_TIME_UNIT_CMD  ,"Querying and Setting WDT time Unit\n"}, /*Tim 20180123*/ 
	...
	while ( info_tbl_ptr->cmd != 0 ) {
        ...
        if ( (ret_val_16 & 0x00ff) != (info_tbl_ptr->cmd | CMD_SUCCESS)) { 
            ...
            switch ( info_tbl_ptr->cmd ) {
                case MAJOR_VER_CMD:  //0x01                   
                case MINOR_VER_CMD:  //0x02
                    fprintf(stdout, "%d\n", (ret_val));
                    break;
                case MODULE_CAP_CMD: //0x03
                    ...
                    fprintf(stdout, "Supported\n"); 
                    break;                        
                case SO_BP_CAP_CMD:  //0x04            
                    so_bp_cap = ret_val;
                    i=0;       
                    while(ret_val&0x01) {i++; ret_val>>=1;}                           
                    fprintf(stdout, "%d bypass pairs\n", i);                     
                    break;                        
                case JO_BP_CAP_CMD:  //0x05                         
                    jo_bp_cap = ret_val;
                    i=0;       
                    while(ret_val&0x01) {i++; ret_val>>=1;}                           
                    fprintf(stdout, "%d bypass pairs\n", i);                     
                    break;                        
                case RT_BP_CAP_CMD: //0x06
                    rt_bp_cap = ret_val;       
                    i=0;
                    while(ret_val&0x01) {i++; ret_val>>=1;}                           
                    fprintf(stdout, "%d bypass pairs\n", i);                     
                    break;                        
                case RT_WDT1_CAP_CMD:  //0x07                      
                case RT_WDT2_CAP_CMD:  //0x08
                    //fprintf(stdout, "%d seconds\n", (int)ret_val);
                    fprintf(stdout, "%d\n", (int)(ret_val)); //Tim 20180320
                    break;                 
                case JO_WDT3_CAP_CMD:   //0x09                  
                    //fprintf(stdout, "%d seconds ( 1 scale = 5 seconds)\n", (int)ret_val*5);
                    fprintf(stdout, "%d\n", (int)(ret_val)*5); //Tim 20180320
                    break;                     
                case SO_BP_SET_CMD:    //0x10                
                    i=1;
                    temp = so_bp_cap;
                    while (temp & 0x01 ) {
                ...
                case RT_BP_SET_CMD:  //0x12
                    ...
                    break;
                case WDT_TIME_UNIT_CMD:   //0x15
                    fprintf(stdout, "%d\n", (int)(ret_val)); //Tim 20180123
                	break;
                case RT_WDT1_STS_CMD:   //0x20                  
                case RT_WDT2_STS_CMD:   //0x30                 
                case JO_WDT3_STS_CMD:   //0x40
                    switch (ret_val) {
                        case WDT_STS_STOP:
                    ...
                    break;                 
                case RT_WDT1_PAIR_CMD:    //0x21                 
                case RT_WDT2_PAIR_CMD:    //0x31                 
                case JO_WDT3_PAIR_CMD:    //0x41
                    i=1;
                    if ( ret_val == 0) {
                ...
                case RT_WDT1_TIMER_CMD:   //0x22                  
                case RT_WDT2_TIMER_CMD:   //0x32           
                    //Tim 20180123 fprintf(stdout, " %d seconds\n", (int)ret_val);       
                    fprintf(stdout, "%d\n", (int)(ret_val)); //Tim 20180123
                    break;                     
                case JO_WDT3_TIMER_CMD:   //0x42
                    //Tim 20180123 fprintf(stdout, " %d seconds ( 1 scale = 5 seconds)\n", (int)ret_val*5);
                    fprintf(stdout, "%d\n", (int)(ret_val)*5); //Tim 20180123
                    break;                     
            }
        }       
        info_tbl_ptr++;   
...
int probe_smbus_controller(void) 
{
  ...
  int ret_value=0;
	...
	{       
        //check the chipset smbus controller is working?  Bus0, Dev31, Fun3    
		addr= (PCIADDR) | ((0x1F <<3)<<8) | (0x03<<8 | 0x08);
		outportl(PCI_INDEX_PORT, addr);
		data_tmp=inportl(PCI_DATA_PORT);
		if((data_tmp!=0xffffffff)&&( (data_tmp>>16)==(0x0c05)))
		{
			//+V1.2.6 Enable SMbus Memory and IO Space 
			addr=(PCIADDR) | ((0x1F <<3)<<8) | (0x03<<8)|0x04;
			outportl(PCI_INDEX_PORT, addr);
			data_tmp=inportl(PCI_DATA_PORT);
			outportl(PCI_DATA_PORT, data_tmp | 0x03);
		    ...
		    //check the chipset smbus controller is working?  skylake platform Bus0, Dev31, Fun4
	       	addr= (PCIADDR) | ((0x1F <<3)<<8) | (0x04<<8 | 0x08);
		    outportl(PCI_INDEX_PORT, addr);
		    data_tmp=inportl(PCI_DATA_PORT);
		    if((data_tmp!=0xffffffff)&&( (data_tmp>>16)==(0x0c05)))
		    {
				//+V1.2.6 Enable SMbus Memory and IO Space 
			    addr=(PCIADDR) | ((0x1F <<3)<<8) | (0x04<<8)|0x04;
			    outportl(PCI_INDEX_PORT, addr);
			    data_tmp=inportl(PCI_DATA_PORT);
			    outportl(PCI_DATA_PORT, data_tmp | 0x03);
			    //try to get smbus base address
			    addr=(PCIADDR) | ((0x1F <<3)<<8) | (0x04<<8)|0x20;
			    outportl(PCI_INDEX_PORT, addr);
			    data_tmp=inportl(PCI_DATA_PORT);
			    return (data_tmp&0x0000fffe);
	     	} 
            else
 		        //check the chipset smbus controller is working?  apollo lake platform Bus0, Dev31, Fun1
	       	    addr= (PCIADDR) | ((0x1F <<3)<<8) | (0x01<<8 | 0x08);
		        outportl(PCI_INDEX_PORT, addr);
		        data_tmp=inportl(PCI_DATA_PORT);
		        if((data_tmp!=0xffffffff)&&( (data_tmp>>16)==(0x0c05)))
		        {
					//+V1.2.6 Enable SMbus Memory and IO Space 
			       addr=(PCIADDR) | ((0x1F <<3)<<8) | (0x01<<8)|0x04;
			       outportl(PCI_INDEX_PORT, addr);
			       data_tmp=inportl(PCI_DATA_PORT);
			       outportl(PCI_DATA_PORT, data_tmp | 0x03);
					
			       //try to get smbus base address
			       addr=(PCIADDR) | ((0x1F <<3)<<8) | (0x01<<8)|0x20;
			       outportl(PCI_INDEX_PORT, addr);
			       data_tmp=inportl(PCI_DATA_PORT);
		           return (data_tmp&0x0000fffe);
	     	    }  
...
int multi_i2c_busses(int *guess_bus) 
{
	...
	while ((de = readdir(dir)) !=NULL) {
		...
		px = fgets(s, 120, f);
		fclose(f);
//V1.2.9 modify		sscanf(de->d_name, "i2c-%d", guess_bus);
//V1.2.9 modify		if ( (strstr(s, "801")) && (bus_count > 1)) {
		if ((strstr(s, "801"))) {
        		fprintf( stdout, "\n*** Best guess: \"%s\" name=%s\n", de->d_name, s);
